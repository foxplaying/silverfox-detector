...
;(function (NS) {
  ...
  NS.isForbiddenSpoofDisplayBrand = function (token) {
    try {
      ...
      // 兜底：抽象对比残片（主路径靠双向校验拒绝；此处仅挡漏网 toast）
      if (/^(?:解析|差异|对比|匹配|校验|核对|核验|比对).{0,4}(?:差异|结果|偏差|误差)?$/.test(s)) return true;
  ...
  NS.isWeakChineseBrandToken = function (token) {
    ...
    // 兜底：抽象对比残片不当品牌（主路径仍是域名↔关键词双向校验；此表只挡漏网展示）
    if (/^(?:解析|差异|对比|匹配|校验|核对|核验|比对|偏差|误差|不一致|一致性|兼容性|可用性|稳定性|正确性|完整性|相关性|相似度|相关度)$/.test(s)) return true;
    if (/^(?:解析|对比|匹配|校验|核对|核验|比对).{0,2}(?:差异|结果|偏差|误差|不一致)?$/.test(s)) return true;
