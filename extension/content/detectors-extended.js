...
;(function (NS) {
  ...
  NS.detectFakeBrandDownloadShell = function () {
    try {
      ...
      const titleOfficial = /官网|官方下载|官方网站|官方正版|官网下载/.test(title);
      if (!titleOfficial) return false;
      const scanBlob = NS.collectPageScriptScanBlob(120000);
      ...
      if (!dynamicUri && !authorMismatch) return false;
      if (!dynamicUri && authorMismatch && !strongAnti && !dupDynamic) return false;
      const hasDlShell = document.querySelectorAll(".download-uri, a.download-uri, .btn-download, [class*='btn-download']").length >= 1 || /download-uri|initDownloadLinks|window\.download_uri/i.test(scanBlob);
      if (!hasDlShell && !dynamicUri) return false;
      const reasons = [];
      if (dynamicUri) reasons.push("动态全局 download_uri 绑定多按钮");
      ...
      if (dynamicUri || dupDynamic) NS.addSignal("多入口共用动态下载地址", 14, "多个下载按钮写入同一 download_uri/安装包地址");
      const target = (() => { try { const a = document.querySelector("a.download-uri[href], a.btn-download[href]"); return (a && (a.getAttribute("href") || a.href)) || "动态下载地址"; } catch { return "动态下载地址"; } })();
      const state = NS.state;
      if (target && !state.protectedTargets.includes(target)) state.protectedTargets.push(target);
      NS.installDownloadGuard(`仿冒官网下载页: ${reasons[0] || "可疑下载壳"}`, { notify: true, href: target, message: NS.formatPackageLabel(target) || "仿冒官网下载", forceNotify: true });
  ...
  NS.detectFakeOfficialDownloadSpa = function () {
    try {
      ...
      if (hasEncryptedDlConfig && productPkgCount === 0 && liveProductPkgs === 0 && (titleClaimsOfficial || ((bodyClaimsOfficial || strongDownloadPitch || downloadPitchText) && (hasDownloadButtons || strongHreflessDlBtns >= 1 || spaRoot)))) {
        ...
        NS.installDownloadGuard("仿冒官网：加密下发安装包，已禁用下载按钮并拦截本页安装包下载", { notify: true, message: "仿冒官网加密下载", forceNotify: true, title: "已拦截可疑安装包" });
        NS.disableAllDownloadIntentControls();
      ...
      if (titleClaimsOfficial && strongHreflessDlBtns >= 1 && strongAnti && !normalContent && productPkgCount === 0 && liveProductPkgs === 0) {
        ...
        NS.installDownloadGuard("仿冒官网：反调试下载页，已禁用下载按钮", { notify: true, message: "仿冒官网反调试下载", forceNotify: true });
        NS.disableAllDownloadIntentControls();
        return true;
      }
    } catch { /* ignore */ }
    ...
    try {
      ...
      document.querySelectorAll("img[src], link[href], script[src], a[href]").forEach((el) => {
        try {
          ...
          const titleTokens = NS.extractLatinBrandTokens(title);
      ...
      if (blobThreat || offsiteBlobLike || multiPlatformSamePackage) {
        ...
        if (target && !targetIsClearSafe) {
          ...
          NS.installDownloadGuard(`仿冒官网：安装包来自第三方对象存储 (${NS.formatPackageLabel(target)})`, { notify: true, href: target, message: NS.formatPackageLabel(target), forceNotify: true });
          NS.disableAllDownloadIntentControls();
          return true;
        }
      }
      if (identityMismatch && (duplicateProductCtas || offsitePackages.length >= 1) && packageHrefs.length >= 1) {
        ...
        if (!target) { /* only clear product */ }
          ...
          if (!state.protectedTargets.includes(target)) state.protectedTargets.push(target);
          NS.installDownloadGuard("仿冒官网：当前域名与页面品牌资源/下载链不一致", { notify: true, href: target, message: NS.formatPackageLabel(target), forceNotify: true });
          NS.disableAllDownloadIntentControls();
          return true;
        }
      }
      if (identityMismatch && brandHostApexes.size >= 1 && ctaEls.length >= 1) {
        ...
        if (topBrand && topBrand !== pageApex && !NS.apexSameBrandFamily(pageApex || location.hostname, topBrand) && !NS.pageIsSameBrandFamilySite(location.hostname, topBrand)) {
          ...
          if (!state.protectedTargets.includes(target)) state.protectedTargets.push(target);
          NS.installDownloadGuard(`仿冒官网：域名与品牌资源不一致（≠ ${topBrand}）`, { notify: true, href: target, message: `${location.hostname} ≠ ${topBrand}`, forceNotify: true });
          NS.disableAllDownloadIntentControls();
          return true;
        }
      }
      if (duplicateProductCtas && packageHrefs.length >= 1) {
        try {
          ...
          if (!sameApexClean) {
            ...
            if (!state.protectedTargets.includes(target)) state.protectedTargets.push(target);
            NS.installDownloadGuard(`仿冒官网：多版本按钮共用安装包 (${NS.formatPackageLabel(target)})`, { notify: true, href: target, message: NS.formatPackageLabel(target), forceNotify: true });
            NS.disableAllDownloadIntentControls();
            return true;
          }
        } catch { /* ignore */ }
  ...
  NS.detectBrandResourceDomainMismatch = function () {
    try {
      ...
      const brandRoot = NS.brandRootKeyFromHost(info.topBrandApex) || (info.topBrandApex.split(".")[0] || "").toLowerCase();
      ...
      const titleHostMismatch = !!corr.mismatch && !NS.pageIsSameBrandFamilySite(host, info.topBrandApex) && !(corr.brandToken && brandRoot && (corr.brandToken === brandRoot || brandRoot.includes(corr.brandToken) || corr.brandToken.includes(brandRoot)));
      let faviconOffBrand = false;
      try {
        document.querySelectorAll('link[rel*="icon"], link[rel="apple-touch-icon"]').forEach((l) => {
          try { const href = l.getAttribute("href") || ""; if (!href) return; const u = new URL(href, location.href); const a = NS.guessApexDomain(u.hostname); if (a && a === info.topBrandApex && a !== info.pageApex && !NS.apexSameBrandFamily(info.pageApex, a)) faviconOffBrand = true; } catch { /* ignore */ }
        });
      } catch { /* ignore */ }
      ...
      try {
        document.querySelectorAll('script[type="application/ld+json"]').forEach((s) => {
          ...
          for (const raw of urls) { try { const a = NS.guessApexDomain(new URL(raw).hostname); if (a && a === info.topBrandApex && a !== info.pageApex && !NS.apexSameBrandFamily(info.pageApex, a)) jsonLdBrandMismatch = true; } catch { /* ignore */ } }
        });
      } catch { /* ignore */ }
      const strong = hostSquatsBrand || faviconOffBrand || jsonLdBrandMismatch || titleHostMismatch || (info.brandAssetHits >= 10 && info.topCount >= 6 && claimsOfficial && dlBtns >= 2 && !NS.pageIsSameBrandFamilySite(host, info.topBrandApex));
      ...
      if (target && !state.protectedTargets.includes(target)) state.protectedTargets.push(target);
      NS.installDownloadGuard(`仿冒官网：域名与品牌资源不一致（${info.pageApex} ≠ ${info.topBrandApex}）`, { notify: true, href: target, message: `${location.hostname} 盗用 ${info.topBrandApex} 资源`, forceNotify: true });
      NS.disableAllDownloadIntentControls();
  ...
  NS.detectRemoteGarblePackageDispatch = function () {
    try {
      ...
      NS.installDownloadGuard(pkg ? `远程乱码安装包: ${label}` : "远程下发乱码安装包", { notify: true, href: pkg || "", message: pkg ? `远程安装包异常: ${label}` : "官网下载页远程下发乱码安装包", title: state.spoofBrand ? `已识别仿冒「${state.spoofBrand}」官网` : "已拦截远程异常安装包", forceNotify: true, guardKind: state.spoofBrand ? "brand-spoof" : "package" });
  ...
  NS.detectDesktopForceDownloadKit = function () {
    try {
      ...
      NS.installDownloadGuard(pkg ? `桌面端强制弹窗下载: ${NS.formatPackageLabel(pkg)}` : "桌面端强制弹窗下载套件", { notify: true, href: pkg || "", message: pkg ? NS.formatPackageLabel(pkg) : "已拦截强制弹窗下载", forceNotify: true, title: "已拦截强制弹窗下载", guardKind: "package" });
      NS.postToHooks({ type: "set-guard", enabled: true });
      NS.disableAllDownloadIntentControls();
      try {
  ...
  NS.detectRemoteDownloadApiBinding = function () {
    ...
    const blob = `${html}\n${scripts}`;
    const apiFetch = /fetch\s*\(\s*['"][^'"]*api\.php[^'"]*['"]/i.test(blob) || /fetch\s*\(\s*['"]https?:\/\/[^'"]+(?:page-admin|download-api|getdown|getlink)[^'"]*['"]/i.test(blob);
    const bindDownload = /download_link|downloadUrl|down_url|download_url/i.test(blob) && (/\.href\s*=/.test(blob) || /querySelectorAll\s*\([^)]*download/i.test(blob));
    if (apiFetch && bindDownload) {
      NS.addSignal("远程API动态绑定下载", 18, "页面通过远程 api 动态写入下载按钮地址，常见于仿冒官网分发模板");
      NS.installDownloadGuard("检测到远程 API 动态绑定下载链接", { notify: true, message: "远程动态下载地址" });
      return true;
    }
    return false;
  };
  ...
  NS.looksLikeSameOriginLandingPageUrl = function (href) {
    ...
    try {
      ...
      return /(?:download|down|dows|install|setup|landing|dl|redirect|clash|verge|\d{3,}down|down\d{2,}|dl\d{2,}|app[_-][a-z0-9]{3,})(?:\.|\/|$)/i.test(url.pathname);
    } catch { return false; }
  };
